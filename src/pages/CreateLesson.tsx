  ...
  if (!user || !['admin', 'host', 'dj', 'instructor', 'media'].includes(profile?.role || '')) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 text-center px-4">
        ...
        <h2 className="text-xl font-bold text-slate-800 dark:text-white">접근 권한이 없습니다</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400">강습 주최자 또는 전문가 계정만 접근할 수 있습니다.</p>
        <button onClick={() => navigate('/')} className="text-indigo-600 font-bold text-sm hover:text-indigo-700 transition-colors">
          메인으로 돌아가기
        </button>
      </div>
    );
    ...
    <div className="min-h-screen pb-32 sm:pb-16">
      ...
      <div className="sticky top-0 z-30 bg-white/90 dark:bg-slate-950/90 backdrop-blur-lg border-b border-slate-200 dark:border-slate-800">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <span className="text-sm font-bold text-slate-900 dark:text-white">새로운 강습 만들기</span>
          <div className="w-16" />
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="max-w-5xl mx-auto px-4 pt-6 pb-4">
          <div className="mb-8">
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">
              새로운 강습 <span className="text-indigo-600">만들기</span>
            </h1>
                  ...
                  {formData.imageUrl ? (
                    ...
                    <div className="aspect-[16/9] flex flex-col items-center justify-center gap-3 text-slate-400 p-6">
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">권장 비율 16:9</p>
                      </div>
                    </div>
                  )}
                </button>
            ...
            <div className="space-y-5">
              ...
              <div className={sectionCls}>
                  ...
                  {formData.tickets.map((t, i) => (
                    <div key={i} className="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                      <input
                        ...
                        placeholder="강습비 명칭 (예: 1회권, 4회 패키지)"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-slate-100 dark:border-slate-800">
                  <div>
                    <label className={labelCls}>
                      <ExternalLink className="w-3.5 h-3.5 inline mr-1 text-slate-400" />온라인 신청 링크
                    </label>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-slate-950/95 backdrop-blur-lg border-t border-slate-200 dark:border-slate-800">
          <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
            ...
            >
              {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> 등록 중...</> : '강습 등록하기 →'}
            </button>
          </div>
        </div>
      </form>
